...
async def create_translation(
    ...
    logging.info(
        f'Inserting translation {word} in {language} into db...'
    )
    await conn[__db_name][__db_collection].insert_one(
        new_translation.model_dump()
    )
    logging.info(
        f"Sample translation {word} in {language} has been inserted into db"
    )
...
async def get_all_translations(
    ...
    projection_map = {key: 1 if key in fields else 0 for key in TranslationDB.model_fields.keys()}
    ...
    if filter:
        translations_cursor = conn[__db_name][__db_collection].find(
            {'word': { "$regex" : filter}}, projection_map
        ).sort({"word": sort_int})
    else:
        translations_cursor = conn[__db_name][__db_collection].find(
            {}, projection_map
...
async def delete_translation(
    ...
    logging.info(
        f"Deleting translation of {word} in {language}..."
    )

    result = await conn[__db_name][__db_collection].\
        delete_one(
        {"$and": [
            {'word': word},
            {'language': language},
    ...
    if result.deleted_count == 0:
        logging.error(
            f"Translation of word {word} in {language} not exist"
        )
    else:
        logging.info(
            f'Translation of {word} in {language} deleted'
        )
